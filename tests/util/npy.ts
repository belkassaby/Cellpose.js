...
export function readNpy(path: string): NpyArray {
  ...
  const major = bytes[6];
  const headerLen = major === 1 ? bytes.readUInt16LE(8) : bytes.readUInt32LE(8);
  ...
  if (!dtypeMatch || !shapeMatch || !forderMatch)
    throw new Error(`malformed npy header: ${header}`);
  const dtype = dtypeMatch[1];
  if (!['<f4', '<u2', '<u4'].includes(dtype as string))
  ...
  const rawBuf = bytes.buffer.slice(
